...
function AssignCourses() {
  ...
  useEffect(() => {
    ...
    const fetchCourses = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/api/get_active_courses');
    ...
    const fetchInstructors = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/api/get_instructors');
        if (response.status === 200) {
  ...
  const handleSave = async () => {
    try {
      await axios.post('http://127.0.0.1:5000/api/assign_instructors', selectedInstructors, {
        headers: {
